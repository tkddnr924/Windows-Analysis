...
function TimelineRow({ entry, currentHostId, onOpen, onRemove }: { entry: BookmarkEntry; currentHostId: string | null; onOpen: (entry: BookmarkEntry) => void; onRemove: (bookmark: Bookmark) => void }) {
  ...
  const hostColor = entry.host.id === currentHostId ? "var(--accent)" : "var(--text-dim)";
  return <article className="dfir-bookmarked-row" style={{ borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", gap: 12, minHeight: 62, marginBottom: 8, padding: "10px 14px", border: "1px solid var(--border)" }}>
    ...
    <span style={{ flexShrink: 0, maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11.5, fontWeight: 700, color: hostColor, border: `1px solid ${hostColor}`, borderRadius: "var(--radius-sm)", padding: "1px 8px" }}>{entry.host.name}</span>
...
function SequenceCanvasEvent({ entry, index, participants, onOpen, accountDirectory }: { entry: SequenceEntry; index: number; participants: SequenceParticipant[]; onOpen: (entry: BookmarkEntry) => void; accountDirectory?: AccountDirectory }) {
  ...
  return <div role="listitem" style={{ position: "absolute", top: top - 26, left: 0, right: 0, height: 58, borderRadius: "var(--radius-sm)", transition: "background .15s ease" }}
    ...
    <button type="button" onClick={() => onOpen(entry)} aria-label={`${formatEvidenceTimestamp(entry.eventTime)} ${connectorText}. ${eventTitle(entry)} 상세 보기`} style={{ position: "absolute", inset: 0, width: "100%", padding: 0, border: "none", background: "transparent", color: "inherit", cursor: "pointer", textAlign: "left" }}>
      ...
      <span aria-hidden="true" style={{ position: "absolute", left: `${labelLeft}%`, top: 0, maxWidth: "min(320px, 44%)", padding: "3px 10px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", color: hasDirectedPeer ? "var(--text)" : "var(--text-dim)", fontSize: 12, fontWeight: 650, boxShadow: "0 1px 3px rgba(0,0,0,.3)" }}>{eventLabel}</span>
      <span style={visuallyHidden}>{connectorText}</span>
    </button>
    <span aria-hidden="true" style={{ position: "absolute", left: `${labelLeft}%`, top: 36, maxWidth: "min(320px, 44%)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: hasDirectedPeer || entry.isLoopback ? lineColor : "var(--text-faint)", fontSize: 10.5, paddingLeft: 2 }}>{connectorText}</span>
  </div>;
}
