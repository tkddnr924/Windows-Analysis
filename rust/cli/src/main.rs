...
use wina_core::parsers::amcache::{parse_amcache, FILES_FIELD_ORDER, FILES_TABLE, PROGRAMS_FIELD_ORDER, PROGRAMS_TABLE};
...
fn flags(args: &[String]) -> std::collections::HashMap<String, String> {
    ...
    while i < args.len() {
        if let Some(k) = args[i].strip_prefix("--") {
            let v = if i + 1 < args.len() && !args[i + 1].starts_with("--") { i += 1; args[i].clone() } else { String::new() };
...
fn main() -> Result<()> {
    ...
    if f.contains_key("list-cases") {
        ...
        let (name, target) = match (name, target) { (Some(n), Some(t)) => (n, t), _ => bail!("--create-host requires --name and --target") };
        let target = std::fs::canonicalize(target).unwrap_or_else(|_| PathBuf::from(target)).to_string_lossy().to_string();
        ...
        let host = f.get("host").ok_or_else(|| anyhow::anyhow!("--run-host requires --host"))?;
        let only: Option<HashSet<String>> = f.get("only").filter(|s| !s.is_empty()).map(|s| s.split(',').map(|x| x.to_string()).collect());
...
fn debug_parse(args: &[String]) -> Result<()> {
    ...
    if args.len() != 3 { bail!("usage: wina {} <input> <out.sqlite>", cmd); }
    let inp = PathBuf::from(&args[1]);
    let out = PathBuf::from(&args[2]);
    match cmd {
        "parse-mft" => { let n = parse_mft_stream(&inp, &out)?; eprintln!("MFT: {} rows", n); }
        "parse-reg" => { let n = parse_hive_stream(&inp, &out)?; eprintln!("Registry: {} rows", n); }
        "parse-evtx" => { let stem = inp.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_else(|| "EventLog".into()); let n = parse_evtx_stream(&inp, &out, &stem)?; eprintln!("EventLog[{}]: {} rows", stem, n); }
        "parse-usn" => { let n = parse_usn_stream(&inp, &out)?; eprintln!("UsnJrnl: {} rows", n); }
        "parse-amcache" => { let (p, fl) = parse_amcache(&inp)?; write_table(&out, PROGRAMS_TABLE, &p, PROGRAMS_FIELD_ORDER)?; write_table(&out, FILES_TABLE, &fl, FILES_FIELD_ORDER)?; eprintln!("Amcache: {} programs, {} files", p.len(), fl.len()); }
        "parse-tasks" => { let r = parse_tasks(&inp)?; write_table(&out, TASK_TABLE, &r, TASK_FIELD_ORDER)?; eprintln!("TaskScheduler: {} tasks", r.len()); }
        "parse-jumplists" => { let r = parse_jumplists(&inp)?; write_table(&out, JUMPLIST_TABLE, &r, JUMPLIST_FIELD_ORDER)?; eprintln!("JumpList: {} entries", r.len()); }
        "parse-srum" => { let t = parse_srum_stream(&inp, &out)?; for (n, c) in &t { eprintln!("  {} : {} rows", n, c); } eprintln!("SRUM: {} tables", t.len()); }
        "parse-wer" => { let r = parse_wer(&inp)?; write_table(&out, WER_TABLE, &r, WER_FIELD_ORDER)?; eprintln!("WER: {} reports", r.len()); }
        "parse-history" => { let t = parse_history(&inp)?; for (n, cols, rows) in &t { write_table_cols(&out, n, rows, cols, &[])?; } eprintln!("BrowserHistory: {} tables", t.len()); }
        "parse-rdpcache" => { let r = parse_rdpcache(&inp)?; write_table(&out, RDP_TABLE, &r, RDP_FIELD_ORDER)?; eprintln!("RdpCache: {} rows", r.len()); }
