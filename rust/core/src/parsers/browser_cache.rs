...
fn block_size(file_type: u32) -> Option<usize> {
    match file_type { 1 => Some(36), 2 => Some(256), 3 => Some(1024), 4 => Some(4096), _ => None }
}
...
fn i32le(b: &[u8], off: usize) -> i32 { u32le(b, off) as i32 }
fn u64le(b: &[u8], off: usize) -> u64 {
    if off + 8 > b.len() { return 0; }
    let mut a = [0u8; 8]; a.copy_from_slice(&b[off..off + 8]); u64::from_le_bytes(a)
}
fn i64le(b: &[u8], off: usize) -> i64 {
    if off + 8 > b.len() { return 0; }
    let mut a = [0u8; 8]; a.copy_from_slice(&b[off..off + 8]); i64::from_le_bytes(a)
}
...
fn chrome_time(value: i64) -> String {
    ...
    match value.checked_mul(10) { Some(t) => fmt_filetime(t), None => String::new() }
}

struct Cache { dir: PathBuf, blocks: HashMap<String, Vec<u8>> }
impl Cache {
    fn new(dir: PathBuf) -> Self { Cache { dir, blocks: HashMap::new() } }
    fn block_bytes(&mut self, name: &str) -> Vec<u8> {
        if let Some(v) = self.blocks.get(name) { return v.clone(); }
    ...
    fn read(&mut self, addr: u32, size: usize) -> Vec<u8> {
        if addr & 0x8000_0000 == 0 { return Vec::new(); }
        let file_type = (addr >> 28) & 0x7;
        if file_type == 0 {
            let f = self.dir.join(format!("f_{:06x}", addr & 0x0FFF_FFFF));
            let data = std::fs::read(f).unwrap_or_default();
            return if size > 0 { data.into_iter().take(size).collect() } else { data };
        }
        let bs = match block_size(file_type) { Some(b) => b, None => return Vec::new() };
        ...
        let length = if size > 0 { size } else { num_blocks as usize * bs };
        if off >= data.len() { return Vec::new(); }
...
fn scan_times(blob: &[u8]) -> Vec<i64> {
    ...
    while off + 8 <= blob.len() {
        ...
        if v > TIME_LO && v < TIME_HI { times.push(v); }
        off += 4;
    }
    times
}

struct Headers { request_time: String, response_time: String, status: String, all_headers: String, map: HashMap<String, String> }

fn parse_headers(stream0: &[u8]) -> Headers {
    let mut out = Headers { request_time: String::new(), response_time: String::new(), status: String::new(), all_headers: String::new(), map: HashMap::new() };
    ...
    if let Some(i) = i {
        let end = find_sub(&stream0[i..], b"\x00\x00").map(|e| i + e).unwrap_or(stream0.len());
        ...
        for (idx, p) in blob.split(|&b| b == 0).enumerate() {
            ...
            if idx == 0 {
                ...
                if sp.len() >= 2 {
                    out.status = if sp.len() > 2 { format!("{} {}", sp[1], sp[2]) } else { sp[1].to_string() };
                }
            } else if let Some((k, v)) = s.split_once(':') {
                out.map.insert(k.trim().to_lowercase(), v.trim().to_string());
            }
        }
...
fn walk_addresses(cache: &mut Cache, index_bytes: &[u8]) -> Vec<u32> {
    if index_bytes.len() < 32 || u32le(index_bytes, 0) != INDEX_MAGIC { return Vec::new(); }
    ...
    for b in 0..table_len as usize {
        ...
        while addr != 0 && (addr & 0x8000_0000) != 0 && !seen.contains(&addr) && addrs.len() < MAX_ENTRIES {
            seen.insert(addr);
            addrs.push(addr);
            let entry = cache.read(addr, ENTRY_SIZE);
            if entry.len() < 8 { break; }
            addr = u32le(&entry, 4);
        }
    }
    addrs
...
fn url_from_key(key: &str) -> String {
    ...
    let pos = a.max(b);
    if pos >= 0 { key[pos as usize..].trim().to_string() } else { key.to_string() }
}

fn entry_row(cache: &mut Cache, addr: u32, account: &str, source: &str) -> Option<Row> {
    let buf = cache.read(addr, ENTRY_SIZE);
    if buf.len() < 96 { return None; }
    let creation_time = u64le(&buf, 24) as i64;
    let key_len = i32le(&buf, 32);
    let long_key = u32le(&buf, 36);
    let data_size = [u32le(&buf, 40), u32le(&buf, 44), u32le(&buf, 48), u32le(&buf, 52)];
    let data_addr = [u32le(&buf, 56), u32le(&buf, 60), u32le(&buf, 64), u32le(&buf, 68)];
    if key_len <= 0 || key_len > 64 * 1024 { return None; }
    ...
    let cache_key = String::from_utf8_lossy(key_bytes.split(|&b| b == 0).next().unwrap_or(&[])).into_owned();
    let url = url_from_key(&cache_key);

    let h = if data_addr[0] != 0 { parse_headers(&cache.read(data_addr[0], data_size[0] as usize)) }
            else { Headers { request_time: String::new(), response_time: String::new(), status: String::new(), all_headers: String::new(), map: HashMap::new() } };
...
fn account_of(path: &Path) -> String {
    ...
    for (i, p) in parts.iter().enumerate() {
        if p.to_uppercase() == "BROWSER" && i + 1 < parts.len() { return parts[i + 1].clone(); }
    }
...
pub fn parse_caches(paths: &[PathBuf]) -> Vec<(String, PathBuf, Vec<Row>)> {
    ...
    for index_path in paths {
        ...
        if parent_name != "Cache_Data" || index_path.to_string_lossy().to_lowercase().contains("code cache") { continue; }
        let index_bytes = match std::fs::read(index_path) { Ok(b) => b, Err(_) => continue };
        if index_bytes.len() < 4 || u32le(&index_bytes, 0) != INDEX_MAGIC { continue; }
        ...
        for addr in walk_addresses(&mut cache, &index_bytes) {
            if let Some(row) = entry_row(&mut cache, addr, &account, &source) { rows.push(row); }
        }
