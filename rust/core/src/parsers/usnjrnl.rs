...
const REASON_FLAGS: &[(u32, &str)] = &[
    ...
    (0x100, "FILE_CREATE"), (0x200, "FILE_DELETE"), (0x400, "EA_CHANGE"), (0x800, "SECURITY_CHANGE"),
    (0x1000, "RENAME_OLD_NAME"), (0x2000, "RENAME_NEW_NAME"), (0x4000, "INDEXABLE_CHANGE"),
    (0x8000, "BASIC_INFO_CHANGE"), (0x10000, "HARD_LINK_CHANGE"), (0x20000, "COMPRESSION_CHANGE"),
    (0x40000, "ENCRYPTION_CHANGE"), (0x80000, "OBJECT_ID_CHANGE"), (0x100000, "REPARSE_POINT_CHANGE"),
    (0x200000, "STREAM_CHANGE"), (0x400000, "TRANSACTED_CHANGE"), (0x800000, "INTEGRITY_CHANGE"),
    (0x80000000, "CLOSE"),
];
const ATTR_FLAGS: &[(u32, &str)] = &[
    (0x1, "READONLY"), (0x2, "HIDDEN"), (0x4, "SYSTEM"), (0x10, "DIRECTORY"), (0x20, "ARCHIVE"),
    (0x80, "NORMAL"), (0x100, "TEMPORARY"), (0x200, "SPARSE_FILE"), (0x400, "REPARSE_POINT"),
    (0x800, "COMPRESSED"), (0x1000, "OFFLINE"), (0x4000, "ENCRYPTED"),
];
fn decode_flags(value: u32, table: &[(u32, &str)]) -> String {
    table.iter().filter(|(b, _)| value & b != 0).map(|(_, n)| *n).collect::<Vec<_>>().join("|")
}

fn u16le(d: &[u8], p: usize) -> u16 { u16::from_le_bytes([d[p], d[p + 1]]) }
fn u32le(d: &[u8], p: usize) -> u32 { u32::from_le_bytes(d[p..p + 4].try_into().unwrap()) }
fn u64le(d: &[u8], p: usize) -> u64 { u64::from_le_bytes(d[p..p + 8].try_into().unwrap()) }
...
pub fn parse_usn_stream(path: &Path, out: &Path) -> Result<usize> {
    ...
    while pos < n {
        ...
        let (file_ref, parent_ref, hdr): (u128, u128, usize) = match major {
            2 => (u64le(&data, pos + 8) as u128, u64le(&data, pos + 16) as u128, pos + 24),
        ...
        let name = {
            ...
            if end <= n {
                let u16s: Vec<u16> = data[start..end].chunks_exact(2).map(|c| u16::from_le_bytes([c[0], c[1]])).collect();
        ...
        row.insert("mft_entry".into(), (file_ref & 0xFFFF_FFFF_FFFF).to_string());
        row.insert("parent_mft_entry".into(), (parent_ref & 0xFFFF_FFFF_FFFF).to_string());
