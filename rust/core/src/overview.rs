...
fn is_live(r: &Row) -> bool { r.get("_recovery").map(|v| v == "live").unwrap_or(true) }
...
pub fn read_all_tables(db: &Path) -> Vec<(String, Vec<Row>)> {
    ...
    let names: Vec<String> = {
        let mut s = match con.prepare("SELECT name FROM sqlite_master WHERE type='table'") { Ok(s) => s, Err(_) => return Vec::new() };
        let it = s.query_map([], |r| r.get::<_, String>(0)).map(|m| m.filter_map(|x| x.ok()).collect::<Vec<_>>()).unwrap_or_default();
        it
    };
    names.into_iter().map(|t| { let rows = read_table(db, &t); (t, rows) }).collect()
}

// --- ScheduledTasks ("작업 스케줄러") ---
const ST_KEYS: &[&str] = &[
    "timestamp", "task_name", "is_microsoft", "actions", "trigger_types", "trigger_start",
    "run_as", "run_level", "logon_type", "enabled", "hidden", "author", "description",
    "uri", "_source_file", "_status", "_error",
];
pub fn build_scheduled_tasks(out_dir: &Path) -> Vec<Row> {
    let src = read_table(&out_dir.join("TASKSCHEDULER").join("TaskScheduler_Tasks.sqlite"), "TaskScheduler_Tasks");
    let mut out = Vec::new();
    for r in &src {
        let uri = r.get("uri").cloned().unwrap_or_default();
        let author = r.get("author").cloned().unwrap_or_default();
        let is_ms = uri.to_lowercase().starts_with("\\microsoft\\") || author.to_lowercase().contains("microsoft");
        let mut row = Row::new();
        for k in ST_KEYS {
            if let Some(v) = r.get(*k) { row.insert((*k).to_string(), v.clone()); }
        }
        row.insert("is_microsoft".into(), if is_ms { "1".into() } else { String::new() });
        out.push(row);
    }
    out
}
...
pub fn build_rdp_cache(out_dir: &Path) -> Vec<Row> {
    read_table(&out_dir.join("RDPCACHE").join("RdpBitmapCache.sqlite"), "RdpBitmapCache")
        .into_iter().filter(|r| r.get("kind").map(|k| k == "fragment").unwrap_or(false)).collect()
}
...
pub fn read_eventlog(out_dir: &Path, needle: &str) -> Vec<Row> {
    ...
    for p in files {
        ...
        if !stem.to_lowercase().contains(needle) { continue; }
...
pub fn parse_eventdata(raw: &str) -> serde_json::Map<String, serde_json::Value> {
    if raw.is_empty() { return serde_json::Map::new(); }
...
fn def_row() -> Row {
    let mut r = Row::new();
    for k in DEF_KEYS { r.insert((*k).to_string(), String::new()); }
    r
}
...
pub fn build_defender(out_dir: &Path) -> Vec<Row> {
    ...
    let mut scans: std::collections::BTreeMap<String, (String, String, String, String)> = std::collections::BTreeMap::new();
    ...
    for r in read_eventlog(out_dir, "defender") {
        ...
        if ["1116", "1117", "1006", "1007", "1015"].contains(&eid.as_str()) {
            ...
            let did = if !did_raw.is_empty() { did_raw } else { format!("{}|{}", ts, jget(&d, "Threat Name")) };
            ...
            for (dst, src) in [
                ...
                let v = jget(&d, src);
                if !v.is_empty() { rec.insert(dst.into(), v); }
            }
            let path = jget(&d, "Path");
            let proc = jget(&d, "Process Name");
            if !path.is_empty() { rec.insert("detail".into(), path); }
            else if !proc.is_empty() { rec.insert("detail".into(), proc); }
            if detected {
                ...
                if !act.is_empty() && act != "해당 없음" { rec.insert("action".into(), act); }
                let rem = jget(&d, "Remediation User");
                if !rem.is_empty() { rec.insert("remediation".into(), rem); }
                let addl = jget(&d, "Additional Actions String");
                if !addl.is_empty() && addl != "No additional actions required" { rec.insert("additional_actions".into(), addl); }
                if rec.get("timestamp").map(|s| s.is_empty()).unwrap_or(true) { rec.insert("timestamp".into(), ts.clone()); }
                if rec.get("record_key").map(|s| s.is_empty()).unwrap_or(true) { rec.insert("record_key".into(), rk.clone()); }
            }
        } else if eid == "5001" {
            ...
            tampering.push((ts, "실시간 보호 구성 변경".into(), "실시간 보호 설정이 변경됨".into(), String::new(), rk, eid));
        } else if eid == "5010" {
            tampering.push((ts, "바이러스 검사 사용 안 함".into(), String::new(), String::new(), rk, eid));
        } else if eid == "5012" {
            tampering.push((ts, "스파이웨어 검사 사용 안 함".into(), String::new(), String::new(), rk, eid));
        } else if eid == "1119" {
            tampering.push((ts, "위협 제거 실패".into(), format!("{} 제거/치료 실패", jget(&d, "Threat Name")), String::new(), rk, eid));
        } else if eid == "1013" {
            let user = format!("{}\\{}", jget(&d, "Domain"), jget(&d, "User"));
            history_cleared.push((ts, user.trim_matches('\\').to_string(), rk));
        } else if eid == "1001" {
            let stype = { let sp = jget(&d, "Scan Parameters"); if !sp.is_empty() { sp } else { let st = jget(&d, "Scan Type"); if !st.is_empty() { st } else { "검사".into() } } };
            let user = format!("{}\\{}", jget(&d, "Domain"), jget(&d, "User"));
            let replace = scans.get(&stype).map(|cur| ts > cur.0).unwrap_or(true);
            if replace { scans.insert(stype, (ts, jget(&d, "Scan Type"), user.trim_matches('\\').to_string(), rk)); }
        } else if eid == "2000" {
            let replace = sig_latest.as_ref().map(|s| ts > s.0).unwrap_or(true);
            if replace { sig_latest = Some((ts, jget(&d, "Current security intelligence Version"), rk)); }
        } else if eid == "5007" {
            let blob = format!("{}{}", jget(&d, "New Value"), jget(&d, "Old Value")).to_lowercase();
            if TAMPER_KEYS.iter().any(|k| blob.contains(k)) {
                tampering.push((ts, "보호 구성 변경".into(), format!("{} → {}", jget(&d, "Old Value"), jget(&d, "New Value")), String::new(), rk, eid));
            }
        }
    }
    ...
    let mut recs: Vec<Row> = det_keys.iter().map(|k| detections.remove(k).unwrap()).collect();
    recs.sort_by(|a, b| a.get("timestamp").cmp(&b.get("timestamp")));
    for mut rec in recs {
        if rec.get("action").map(|s| s.is_empty()).unwrap_or(true) { rec.insert("action".into(), "탐지만 됨".into()); }
        out.push(rec);
    }
    ...
    for (ts, is_on, rk) in rt_events {
        match prev {
            None => {
                prev = Some(is_on);
                if !is_on { tampering.push((ts, "실시간 보호 사용 안 함".into(), "Defender 실시간 보호가 해제됨".into(), String::new(), rk, "5001".into())); }
            }
            Some(p) if is_on == p => {}
            Some(_) => {
                prev = Some(is_on);
                if is_on { tampering.push((ts, "실시간 보호 복원".into(), "Defender 실시간 보호가 다시 켜짐".into(), String::new(), rk, "5000".into())); }
                else { tampering.push((ts, "실시간 보호 사용 안 함".into(), "Defender 실시간 보호가 해제됨".into(), String::new(), rk, "5001".into())); }
            }
        }
    }
    ...
    if !history_cleared.is_empty() {
        ...
        r.insert("detail".into(), format!("Defender 기록이 삭제됨 (총 {}회, 최근 시각 표시)", history_cleared.len()));
...
fn ed_field(ed: &serde_json::Map<String, serde_json::Value>, names: &[&str]) -> String {
    ...
    if let Some(serde_json::Value::Object(x)) = ed.get("EventXML") { sources.push(x); }
    sources.push(ed);
    for src in &sources {
        for n in names {
...
pub fn build_smb_history(out_dir: &Path) -> Vec<Row> {
    ...
    for r in read_eventlog(out_dir, "") {
        ...
        if provider == "Microsoft-Windows-Security-Auditing" && (eid == "4624" || eid == "4625") {
            if ed_field(&ed, &["LogonType"]) != "3" { continue; }
            ...
            description = if eid == "4624" { "네트워크 로그온 성공 (SMB 등)" } else { "네트워크 로그온 실패 (SMB 등)" }.to_string();
        } else if provider == "Microsoft-Windows-SMBServer" && (eid == "551" || eid == "1009") {
            let inner = match ed.get("EventData") { Some(serde_json::Value::Object(m)) => m.clone(), _ => ed.clone() };
        ...
        if remote.is_empty() || remote == "-" { continue; }
        let mut row = Row::new();
        row.insert("timestamp".into(), r.get("timestamp").cloned().unwrap_or_default());
        row.insert("direction".into(), "inbound".into());
        row.insert("remote_address".into(), std::mem::take(&mut remote));
        row.insert("account".into(), bare_account(std::mem::take(&mut account).as_str()));
        ...
        row.insert("record_key".into(), r.get("_record_key").cloned().unwrap_or_default());
        rows.push(row);
    }
    rows
}

struct RdpSpec { direction: &'static str, result: &'static str, description: &'static str, addr: &'static [&'static str], acct: &'static [&'static str], lt10: bool }
fn rdp_spec(provider: &str, eid: &str) -> Option<RdpSpec> {
    let s = |direction, result, description, addr: &'static [&'static str], acct: &'static [&'static str], lt10| Some(RdpSpec { direction, result, description, addr, acct, lt10 });
    ...
    match (provider, eid) {
        (SEC, "4624") => s("inbound", "성공", "RDP 로그온 성공", &["IpAddress"], &["TargetUserName"], true),
        (SEC, "4625") => s("inbound", "실패", "RDP 로그온 실패", &["IpAddress"], &["TargetUserName"], true),
        (RCM, "1149") => s("inbound", "성공", "RDP 네트워크 인증 성공(로그인 화면 도달)", &["Param3"], &["Param1"], false),
        (LSM, "21") => s("inbound", "성공", "RDP 세션 로그온", &["Address"], &["User"], false),
        (LSM, "25") => s("inbound", "성공", "RDP 세션 재연결", &["Address"], &["User"], false),
        (LSM, "22") => s("inbound", "정보", "RDP 셸 시작", &["Address"], &["User"], false),
        (LSM, "23") => s("inbound", "정보", "RDP 세션 로그오프", &["Address"], &["User"], false),
        (LSM, "24") => s("inbound", "정보", "RDP 세션 연결 끊김", &["Address"], &["User"], false),
        (LSM, "39") => s("inbound", "정보", "RDP 세션 연결 끊김(다른 세션에 의해)", &["Address"], &["User"], false),
        (LSM, "40") => s("inbound", "정보", "RDP 세션 연결 끊김", &["Address"], &["User"], false),
        (AX, "1024") => s("outbound", "정보", "RDP 아웃바운드 연결 시도", &["Value"], &[], false),
        (AX, "1102") => s("outbound", "정보", "RDP 아웃바운드 서버 주소", &["Value"], &[], false),
        (AX, "1025") => s("outbound", "성공", "RDP 아웃바운드 연결됨", &[], &[], false),
        (AX, "1026") => s("outbound", "정보", "RDP 아웃바운드 연결 끊김", &[], &[], false),
        _ => None,
    }
}
...
fn ts_epoch(ts: &str) -> Option<f64> {
    ...
    for (fmt, cut) in [("%Y-%m-%d %H:%M:%S%.f", 23usize), ("%Y-%m-%d %H:%M:%S", 19usize)] {
...
pub fn build_remote_desktop_history(out_dir: &Path) -> Vec<Row> {
    ...
    let mut sess_info: std::collections::HashMap<String, [Option<String>; 2]> = std::collections::HashMap::new();
    let mut lsm_pending: Vec<(usize, String)> = Vec::new();

    for r in read_eventlog(out_dir, "") {
        let provider = r.get("Provider").cloned().unwrap_or_default();
        let eid = r.get("EventID").cloned().unwrap_or_default();
        let spec = match rdp_spec(&provider, &eid) { Some(s) => s, None => continue };
        let ed = parse_eventdata(r.get("EventData").map(|s| s.as_str()).unwrap_or(""));
        if spec.lt10 && ed.get("LogonType").map(vstr).unwrap_or_default() != "10" { continue; }
        let addr = if !spec.addr.is_empty() { ed_field(&ed, spec.addr) } else { String::new() };
        let acct = if !spec.acct.is_empty() { bare_account(&ed_field(&ed, spec.acct)) } else { String::new() };
        let mut row = Row::new();
        row.insert("timestamp".into(), r.get("timestamp").cloned().unwrap_or_default());
        ...
        if provider == LSM {
            let src: &serde_json::Map<String, serde_json::Value> = match ed.get("EventXML") { Some(serde_json::Value::Object(x)) => x, _ => &ed };
            let skey = ["SessionID", "Session", "TargetSession"].iter()
            ...
            if let Some(skey) = skey {
                ...
                if !addr.is_empty() { info[0] = Some(addr); }
                if !acct.is_empty() { info[1] = Some(acct); }
    ...
    for (idx, skey) in lsm_pending {
        if let Some(info) = sess_info.get(&skey) {
            if rows[idx].get("remote_address").map(|s| s.is_empty()).unwrap_or(true) {
                if let Some(a) = &info[0] { rows[idx].insert("remote_address".into(), a.clone()); }
            }
            if rows[idx].get("account").map(|s| s.is_empty()).unwrap_or(true) {
                if let Some(a) = &info[1] { rows[idx].insert("account".into(), a.clone()); }
            }
        }
    }
    ...
    for i in order {
        if rows[i].get("direction").map(|s| s.as_str()) != Some("outbound") { continue; }
        let t = ts_epoch(rows[i].get("timestamp").map(|s| s.as_str()).unwrap_or(""));
        if !rows[i].get("remote_address").map(|s| s.is_empty()).unwrap_or(true) {
            last_out = Some((t, rows[i].get("remote_address").cloned().unwrap_or_default()));
        } else if let Some((Some(lt), addr)) = &last_out {
            if let Some(t) = t { if t - lt >= 0.0 && t - lt <= WINDOW { rows[i].insert("remote_address".into(), addr.clone()); } }
        }
    }
    rows
}
...
fn read_registry_all(out_dir: &Path) -> Vec<Row> {
    ...
    files.sort();
    let mut out = Vec::new();
    for p in files { out.extend(read_table(&p, "Registry").into_iter().filter(is_live)); }
    out
}

fn basename_win(path: &str) -> String {
    if path.is_empty() { return String::new(); }
    path.replace('/', "\\").rsplit('\\').next().unwrap_or("").to_string()
}
...
fn user_map(out_dir: &Path) -> std::collections::HashMap<String, String> {
    ...
    for r in read_registry_all(out_dir) {
        ...
        if kp.to_lowercase().contains("\\profilelist\\s-") && r.get("value_name").map(|s| s == "ProfileImagePath").unwrap_or(false) {
            let sid = kp.rsplit('\\').next().unwrap_or("").to_string();
            let name = basename_win(&r.get("value_data").cloned().unwrap_or_default());
            if !sid.is_empty() && !name.is_empty() { umap.insert(sid, name); }
        }
    }
    umap
}
...
fn all_strings(v: &serde_json::Value, out: &mut Vec<String>) {
    match v {
        ...
        serde_json::Value::Object(m) => for x in m.values() { all_strings(x, out); },
        serde_json::Value::Array(a) => for x in a { all_strings(x, out); },
        _ => {}
    }
}
...
fn ctx_value(ctx: &str, tokens: &[&str]) -> String {
    for line in ctx.lines() {
        ...
        if tokens.iter().any(|t| !t.is_empty() && label.contains(t)) { return value.trim().to_string(); }
    }
    String::new()
}
...
fn exe_from_host(host: &str) -> String {
    ...
    let path = if let Some(rest) = h.strip_prefix('"') {
        match rest.find('"') { Some(e) => &rest[..e], None => rest }
    } else {
        h.split(' ').next().unwrap_or(h)
    };
    let b = basename_win(path);
    if b.is_empty() { path.to_string() } else { b }
}

fn first_line(text: &str, limit: usize) -> String {
    for line in text.lines() {
        let s = line.trim();
        if !s.is_empty() { return s.chars().take(limit).collect(); }
    }
...
fn find_token(blob: &str, prefix: &str) -> String {
    ...
    if let Some(pos) = blob.find(prefix) {
        ...
        if !line.is_empty() { return line.trim().to_string(); }
    }
    String::new()
}
...
pub fn build_powershell_history(out_dir: &Path) -> Vec<Row> {
    ...
    let account_for = |sid: &str| -> String { match umap.get(sid) { Some(n) if !n.is_empty() => n.clone(), _ => sid.to_string() } };
    let mut rows: Vec<Row> = Vec::new();
    // ScriptBlockId -> (parts: BTreeMap<i64,String>, timestamp, sid, pid, rk, path)
    let mut block_keys: Vec<String> = Vec::new();
    let mut blocks: std::collections::HashMap<String, (std::collections::BTreeMap<i64, String>, String, String, String, String, String)> = std::collections::HashMap::new();
        ...
        if provider == "Microsoft-Windows-PowerShell" && event_id == "4104" {
            ...
            if !sbid.is_empty() && total > 1 {
                if !blocks.contains_key(&sbid) {
                    ...
                    blocks.insert(sbid.clone(), (std::collections::BTreeMap::new(), ts.clone(), sid.clone(), pid.clone(), rk.clone(), path.clone()));
                }
            ...
            let payload = ed.get("Payload").map(vstr).unwrap_or_default().trim().to_string();
...
fn bh_row(pairs: &[(&str, String)]) -> Row {
    let mut r = Row::new();
    for k in BH_KEYS { r.insert((*k).to_string(), String::new()); }
...
fn chrome_time(value: &str) -> String {
    let v: i64 = match value.trim().parse() { Ok(v) => v, Err(_) => return String::new() };
    if v <= 0 { return String::new(); }
    match v.checked_mul(10) { Some(ticks) => crate::time::fmt_filetime(ticks), None => String::new() }
}
...
fn url_decode(url: &str) -> String {
    ...
    let mut i = 0;
    let hexval = |c: u8| -> Option<u8> { match c { b'0'..=b'9' => Some(c - b'0'), b'a'..=b'f' => Some(c - b'a' + 10), b'A'..=b'F' => Some(c - b'A' + 10), _ => None } };
    while i < b.len() {
        if b[i] == b'%' && i + 2 < b.len() {
...
fn human_bytes(value: &str) -> String {
    let n: i64 = match value.trim().parse() { Ok(v) => v, Err(_) => return String::new() };
    if n < 0 { return String::new(); }
    let units = ["B", "KB", "MB", "GB", "TB"];
    let mut f = n as f64;
    for (idx, u) in units.iter().enumerate() {
        if f < 1024.0 || idx == units.len() - 1 {
            return if *u == "B" { format!("{} {}", f as i64, u) } else { format!("{:.1} {}", f, u) };
        }
        f /= 1024.0;
    }
    format!("{} B", n)
...
fn browser_sources(out_dir: &Path, category: &str, suffix: &str) -> Vec<(String, std::path::PathBuf)> {
...
pub fn build_browser_history(out_dir: &Path) -> Vec<Row> {
    ...
    for (account, db) in browser_sources(out_dir, "BROWSERCACHE", "_Chrome_Cache") {
        for c in read_table(&db, "CacheEntries") {
            ...
            let url = g("url");
            if ts.is_empty() || url.is_empty() { continue; }
    ...
    for (account, db) in browser_sources(out_dir, "BROWSER", "_Chrome_History") {
        for u in read_table(&db, "urls") {
            ...
            let count = |v: String| if v.is_empty() || v == "0" { String::new() } else { v };
            rows.push(bh_row(&[("account", account.clone()), ("kind", "visit".into()),
                ("timestamp", chrome_time(&g("last_visit_time"))), ("title", g("title")),
                ("url", url_decode(&raw)), ("url_raw", raw.clone()),
                ("visit_count", count(g("visit_count"))), ("typed_count", count(g("typed_count")))]));
        }
        for d in read_table(&db, "downloads") {
            let g = |k: &str| d.get(k).cloned().unwrap_or_default();
            let tgt = { let t = g("target_path"); if !t.is_empty() { t } else { g("current_path") } };
            let src = { let a = g("tab_url"); if !a.is_empty() { a } else { let b = g("referrer"); if !b.is_empty() { b } else { g("site_url") } } };
            ...
            let size_raw = { let t = g("total_bytes"); if !t.is_empty() { t } else { g("received_bytes") } };
            rows.push(bh_row(&[("account", account.clone()), ("kind", "download".into()),
...
fn ti_row(pairs: &[(&str, String)]) -> Row {
    let mut r = Row::new();
    for k in TI_KEYS { r.insert((*k).to_string(), String::new()); }
    for (k, v) in pairs { r.insert((*k).to_string(), v.clone()); }
    r
}

fn le_u16(b: &[u8], off: usize) -> u16 { if off + 2 > b.len() { 0 } else { u16::from_le_bytes([b[off], b[off + 1]]) } }
fn le_u32(b: &[u8], off: usize) -> u32 { if off + 4 > b.len() { 0 } else { u32::from_le_bytes([b[off], b[off + 1], b[off + 2], b[off + 3]]) } }
fn le_u64(b: &[u8], off: usize) -> u64 { if off + 8 > b.len() { 0 } else { let mut a = [0u8; 8]; a.copy_from_slice(&b[off..off + 8]); u64::from_le_bytes(a) } }
...
fn filetime(value: u64) -> String { if value == 0 { String::new() } else { crate::time::fmt_filetime(value as i64) } }

fn filetime_hex(hexstr: &str) -> String { let b = unhex(hexstr); if b.len() >= 8 { filetime(le_u64(&b, 0)) } else { String::new() } }
...
fn systemtime_hex(hexstr: &str) -> String {
    ...
    if y == 0 { String::new() } else { format!("{:04}-{:02}-{:02} {:02}:{:02}:{:02}", y, mo, d, h, mi, s) }
}

/// Unix epoch seconds -> KST "YYYY-MM-DD HH:MM:SS.000".
fn unixdate(value: &str) -> String {
    let ts: i64 = match value.trim().parse() { Ok(v) => v, Err(_) => return String::new() };
...
/// All rows of registry hives whose file stem uppercases to `name`.
fn registry_hive(out_dir: &Path, name: &str) -> Vec<Row> {
    let dir = out_dir.join("REGISTRY");
    let mut files: Vec<_> = match std::fs::read_dir(&dir) {
        Ok(rd) => rd.filter_map(|e| e.ok().map(|e| e.path()))
            .filter(|p| p.extension().map(|x| x == "sqlite").unwrap_or(false)).collect(),
...
fn reg_pick(rows: &[Row], key_suffix: &str, value_name: &str) -> String {
    ...
    let hits: Vec<&Row> = rows.iter().filter(|r| {
        r.get("key_path").map(|k| k.to_lowercase().ends_with(&ks)).unwrap_or(false)
            && r.get("value_name").map(|v| v == value_name).unwrap_or(false)
            && r.get("value_data").map(|v| !v.is_empty()).unwrap_or(false)
    }).collect();
    for r in &hits {
        if r.get("key_path").map(|k| k.to_lowercase().contains("controlset001")).unwrap_or(false) {
            return r.get("value_data").cloned().unwrap_or_default();
        }
    }
    hits.first().map(|r| r.get("value_data").cloned().unwrap_or_default()).unwrap_or_default()
}

const OS_VALUES: &[&str] = &["ProductName", "EditionID", "DisplayVersion", "CurrentBuild", "RegisteredOwner", "InstallDate"];
const ACB: &[(u16, &str)] = &[
...
fn ti_system_info(software: &[Row], system: &[Row]) -> Vec<Row> {
    ...
    for r in software {
        ...
        if kp.to_lowercase().ends_with(cv) && OS_VALUES.contains(&name.as_str()) {
            ...
            let val = if name == "InstallDate" { unixdate(&data) } else { data };
            if name == "InstallDate" && val.is_empty() { continue; }
            seen.insert(name.clone());
            rows.push(ti_row(&[("category", "SystemInfo".into()), ("name", name), ("value", val), ("source_artifact", "SOFTWARE".into())]));
        }
    }
    let cn = reg_pick(system, "\\control\\computername\\computername", "ComputerName");
    if !cn.is_empty() { rows.push(ti_row(&[("category", "SystemInfo".into()), ("name", "ComputerName".into()), ("value", cn), ("source_artifact", "SYSTEM".into())])); }
    let tz = reg_pick(system, "\\control\\timezoneinformation", "TimeZoneKeyName");
    if !tz.is_empty() { rows.push(ti_row(&[("category", "SystemInfo".into()), ("name", "TimeZone".into()), ("value", tz), ("source_artifact", "SYSTEM".into())])); }
    let sd = reg_pick(system, "\\control\\windows", "ShutdownTime");
    if !sd.is_empty() { rows.push(ti_row(&[("category", "SystemInfo".into()), ("name", "LastShutdownTime".into()), ("value", filetime_hex(&sd)), ("source_artifact", "SYSTEM".into())])); }
    rows
}

fn sam_f(b: &[u8]) -> Option<HashMap<&'static str, String>> {
    if b.len() < 0x44 { return None; }
    ...
    m.insert("disabled", if acb & 0x0001 != 0 { "예".into() } else { "아니오".into() });
    m.insert("account_flags", ACB.iter().filter(|(bit, _)| acb & bit != 0).map(|(_, n)| *n).collect::<Vec<_>>().join(", "));
    Some(m)
}

fn sam_v(b: &[u8]) -> (String, String) {
    if b.len() < 0xCC { return (String::new(), String::new()); }
    let field = |idx: usize| -> String {
        ...
        if start >= b.len() { return String::new(); }
        let raw = &b[start..end];
...
fn special_accounts(software: &[Row]) -> HashMap<String, String> {
    ...
    for r in software {
        ...
        if kp.to_lowercase().ends_with(mark) && !vn.is_empty() {
            out.insert(vn.to_lowercase(), r.get("value_data").cloned().unwrap_or_default().trim().to_string());
        }
    }
    out
}
...
fn well_known_sid(sid: &str) -> Option<&'static str> {
    match sid { "S-1-5-18" => Some("LocalSystem"), "S-1-5-19" => Some("LocalService"), "S-1-5-20" => Some("NetworkService"), _ => None }
}
...
fn ti_accounts(sam: &[Row], software: &[Row]) -> Vec<Row> {
    ...
    for r in software {
        let kp = r.get("key_path").cloned().unwrap_or_default();
        if kp.to_lowercase().contains("\\profilelist\\s-") && r.get("value_name").map(|v| v == "ProfileImagePath").unwrap_or(false) {
            let sid = kp.rsplit('\\').next().unwrap_or("").to_string();
            if !profiles.contains_key(&sid) { profile_keys.push(sid.clone()); }
            profiles.insert(sid.clone(), r.get("value_data").cloned().unwrap_or_default());
            if sid.starts_with("S-1-5-21-") && prefix.is_empty() {
                if let Some(p) = sid.rfind('-') { prefix = sid[..p].to_string(); }
            }
        }
    }
    ...
    for r in sam {
        ...
        if let Some(i) = low.find(nmark) {
            ...
            if !tail.is_empty() && !tail.contains('\\') {
                names_created.insert(tail.to_lowercase(), r.get("last_write").cloned().unwrap_or_default());
            }
            continue;
        }
        let umark = "\\users\\";
        if let Some(i) = low.rfind(umark) {
            ...
            if tail.len() == 8 && hexset(tail) {
                if !users.contains_key(tail) { user_keys.push(tail.to_string()); users.insert(tail.to_string(), (Vec::new(), Vec::new())); }
    ...
    for rid_hex in &user_keys {
        ...
        let sid = if !prefix.is_empty() { format!("{}-{}", prefix, rid) } else { String::new() };
        let path = profiles.get(&sid).cloned().unwrap_or_default();
        let created = names_created.get(&username.to_lowercase()).cloned().unwrap_or_default();
        let special = match hidden.get(&username.to_lowercase()) {
            None => String::new(),
            Some(sp) if sp == "0" || sp == "0x0" => "예 (로그온 화면 숨김)".into(),
            Some(sp) => format!("UserList 값 {}", sp),
        };
        let g = |k: &str| f.as_ref().and_then(|m| m.get(k)).cloned().unwrap_or_default();
        let last_login = g("last_login");
        if !sid.is_empty() { emitted.insert(sid.clone()); }
        rows.push(ti_row(&[
            ("category", "Account".into()), ("name", sid.clone()), ("value", path.clone()), ("source_artifact", "SAM".into()),
            ("username", username), ("full_name", full_name),
            ("rid", rid.to_string()), ("rid_sam", g("rid")),
            ("home_directory", path.clone()), ("created", created.clone()),
            ("last_login", last_login.clone()), ("password_last_set", g("password_last_set")),
            ("last_failed_login", g("last_failed_login")), ("login_count", g("login_count")),
            ("failed_login_count", g("failed_login_count")), ("disabled", g("disabled")),
            ("account_flags", g("account_flags")), ("special_account", special),
            ("timestamp", if !last_login.is_empty() { last_login } else { created }),
        ]));
    }

    for sid in &profile_keys {
        if emitted.contains(sid) { continue; }
        let path = profiles.get(sid).cloned().unwrap_or_default();
        let uname = well_known_sid(sid).map(|s| s.to_string())
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| { let b = basename_win(path.trim_end_matches('\\')); if !b.is_empty() { b } else { sid.clone() } });
        rows.push(ti_row(&[
            ("category", "Account".into()), ("name", sid.clone()), ("value", path.clone()),
            ("source_artifact", "ProfileList".into()), ("username", uname), ("home_directory", path),
        ]));
    }
    rows
}
...
fn iplist(value: &str) -> String {
    ...
    if s.starts_with('[') {
        match serde_json::from_str::<Vec<String>>(s) {
            Ok(v) => v.into_iter().filter(|x| !x.is_empty() && x != "0.0.0.0").collect::<Vec<_>>().join(", "),
            Err(_) => s.to_string(),
        }
    } else if s == "0.0.0.0" { String::new() } else { s.to_string() }
}
...
fn ti_networks(software: &[Row]) -> Vec<Row> {
    ...
    for r in software {
        ...
        if kp.to_lowercase().contains("\\networklist\\profiles\\{") {
            if !prof.contains_key(&kp) { keys.push(kp.clone()); prof.insert(kp.clone(), (String::new(), String::new())); }
            let e = prof.get_mut(&kp).unwrap();
            match r.get("value_name").map(|s| s.as_str()) {
                Some("ProfileName") => e.0 = r.get("value_data").cloned().unwrap_or_default(),
                Some("DateLastConnected") => e.1 = systemtime_hex(&r.get("value_data").cloned().unwrap_or_default()),
                _ => {}
            }
        }
    }
    keys.iter().filter_map(|k| { let (name, when) = prof.get(k).unwrap(); if name.is_empty() { None } else {
        Some(ti_row(&[("category", "Network".into()), ("name", "연결한 네트워크".into()), ("value", name.clone()), ("timestamp", when.clone()), ("source_artifact", "NetworkList".into())]))
    }}).collect()
}
...
fn ti_network_interfaces(system: &[Row]) -> Vec<Row> {
    ...
    for r in system {
        ...
        if let Some(i) = low.find(mark) {
            ...
            if !ifaces.contains_key(&guid) { guids.push(guid.clone()); ifaces.insert(guid.clone(), HashMap::new()); }
            ifaces.get_mut(&guid).unwrap().insert(r.get("value_name").cloned().unwrap_or_default(), r.get("value_data").cloned().unwrap_or_default());
        }
    }
    let mut rows = Vec::new();
    for guid in &guids {
        let d = &ifaces[guid];
        let g = |k: &str| d.get(k).cloned().unwrap_or_default();
        let ip = { let a = iplist(&g("IPAddress")); if !a.is_empty() { a } else { g("DhcpIPAddress") } };
        if ip.is_empty() || ip == "0.0.0.0" { continue; }
        let dhcp = g("EnableDHCP");
        let or = |a: String, b: String| if !a.is_empty() { a } else { b };
        rows.push(ti_row(&[
            ("category", "NetworkInterface".into()), ("name", guid.clone()), ("value", ip), ("source_artifact", "Tcpip".into()),
            ("subnet_mask", or(iplist(&g("SubnetMask")), g("DhcpSubnetMask"))),
            ("gateway", or(iplist(&g("DefaultGateway")), g("DhcpDefaultGateway"))),
            ("dns_server", or(g("NameServer"), g("DhcpNameServer"))),
            ("dhcp_server", g("DhcpServer")),
            ("dhcp_enabled", if dhcp == "1" { "예".into() } else if dhcp == "0" { "아니오".into() } else { String::new() }),
...
pub fn build_target_info(out_dir: &Path) -> Vec<Row> {
    let software = registry_hive(out_dir, "SOFTWARE");
    let system = registry_hive(out_dir, "SYSTEM");
    let sam = registry_hive(out_dir, "SAM");
    let mut rows = Vec::new();
...
fn eh_row(pairs: &[(&str, String)]) -> Row {
    let mut r = Row::new();
    for k in ROW_KEYS { r.insert((*k).to_string(), String::new()); }
    for (k, v) in pairs { r.insert((*k).to_string(), v.clone()); }
    r
}
...
fn eh_from_userassist(out_dir: &Path) -> Vec<Row> {
    ...
    for p in files {
        ...
        for r in read_table(&p, "Registry").into_iter().filter(is_live) {
            ...
            if UA_MARKERS.contains(&decoded.as_str()) { continue; }
            let data = unhex(&r.get("value_data").cloned().unwrap_or_default());
            let (mut run_count, mut focus_count, mut focus_ms, mut ts) = (String::new(), String::new(), String::new(), String::new());
            if data.len() >= 68 {
...
fn eh_from_bam(out_dir: &Path) -> Vec<Row> {
    ...
    for r in registry_hive(out_dir, "SYSTEM") {
        ...
        if !exe.to_lowercase().ends_with(".exe") { continue; }
...
pub fn build_execution_history(out_dir: &Path) -> Vec<Row> {
    ...
    rows.extend(eh_from_bam(out_dir));
    rows.extend(eh_from_prefetch(out_dir));
    rows
}

// --- RegistryFindings ("레지스트리 특이사항") ---
const RF_KEYS: &[&str] = &["timestamp", "category", "name", "value", "status", "detail", "key_path", "source", "command", "user", "subtype"];
fn rf_row(pairs: &[(&str, String)]) -> Row {
    let mut r = Row::new();
    for k in RF_KEYS { r.insert((*k).to_string(), String::new()); }
    for (k, v) in pairs { r.insert((*k).to_string(), v.clone()); }
    r
}

/// (hive file stem, live rows) for every dumped registry hive.
fn registry_hives_named(out_dir: &Path) -> Vec<(String, Vec<Row>)> {
    let dir = out_dir.join("REGISTRY");
    let mut files: Vec<_> = match std::fs::read_dir(&dir) {
        Ok(rd) => rd.filter_map(|e| e.ok().map(|e| e.path()))
            .filter(|p| p.extension().map(|x| x == "sqlite").unwrap_or(false)).collect(),
    ...
    files.into_iter().map(|p| {
        let stem = p.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default();
        let rows: Vec<Row> = read_table(&p, "Registry").into_iter().filter(is_live).collect();
        (stem, rows)
    }).collect()
}
...
fn hive_user(fname: &str) -> String {
    ...
    if up == "SOFTWARE" { return "(시스템)".into(); }
    if up == "DEFAULT" { return ".DEFAULT".into(); }
    if up.ends_with("NTUSER.DAT") {
        return if up.ends_with("_NTUSER.DAT") { fname[..fname.len() - "_NTUSER.DAT".len()].to_string() } else { fname.to_string() };
    }
    fname.to_string()
}
...
fn parse_shimcache(data: &[u8]) -> Vec<(String, u64)> {
    ...
    let mut off: Option<usize> = if header > 0 && header < data.len() && data.get(header..header + 4) == Some(b"10ts") {
        Some(header)
    } else { find_sub_local(data, b"10ts") };
    while let Some(o) = off {
        if o + 12 > data.len() || data.get(o..o + 4) != Some(b"10ts") { break; }
        ...
        if entry.len() >= 2 {
            ...
            if 2 + path_len + 8 <= entry.len() {
                ...
                let u16s: Vec<u16> = path_bytes.chunks_exact(2).map(|c| u16::from_le_bytes([c[0], c[1]])).collect();
                let path = String::from_utf16_lossy(&u16s);
                let ft = le_u64(entry, 2 + path_len);
                if !path.is_empty() && (path.contains('\\') || path.contains(':')) { entries.push((path, ft)); }
            }
        }
...
fn rf_credential_protection(system: &[Row]) -> Vec<Row> {
    ...
    if wd == "1" {
        ...
        rows.push(rf_row(&[("category", "자격 증명 보호".into()), ("name", "WDigest UseLogonCredential".into()), ("value", "0 (사용 안 함)".into()), ("status", "정상".into()),
    ...
    if ppl == "1" || ppl == "2" {
        rows.push(rf_row(&[("category", "자격 증명 보호".into()), ("name", "LSASS 보호(RunAsPPL)".into()), ("value", format!("{} (사용)", ppl)), ("status", "정상".into()),
            ("detail", "LSASS가 PPL(보호 프로세스)로 실행됨 — 자격증명 덤프 난이도 상승".into()),
            ("key_path", "…\\Control\\Lsa".into()), ("source", "SYSTEM".into())]));
    } else {
        rows.push(rf_row(&[("category", "자격 증명 보호".into()), ("name", "LSASS 보호(RunAsPPL)".into()), ("value", "미설정".into()), ("status", "주의".into()),
            ("detail", "RunAsPPL 미설정 — LSASS가 보호 프로세스로 실행되지 않아 자격증명 덤프에 노출".into()),
            ("key_path", "…\\Control\\Lsa".into()), ("source", "SYSTEM".into())]));
    }
    rows
}
...
fn share_path(value: &str) -> String {
    ...
    for it in items {
        if it.to_lowercase().starts_with("path=") { return it[5..].to_string(); }
    }
    String::new()
}
...
fn rf_shares(system: &[Row]) -> Vec<Row> {
    ...
    if !start.is_empty() {
        ...
        let vmap = match start.as_str() { "2" => "자동", "3" => "수동", "4" => "사용 안 함", _ => start.as_str() };
        rows.push(rf_row(&[("category", "공유 폴더".into()), ("name", "Server 서비스(LanmanServer)".into()), ("value", vmap.into()),
            ("status", if enabled { "정보".into() } else { "정상".into() }),
            ("detail", format!("파일/프린터 공유 서비스{}", if enabled { "가 실행됩니다 (공유 가능 상태)." } else { "가 비활성화되어 있습니다." })),
            ("key_path", "…\\Services\\LanmanServer".into()), ("source", "SYSTEM".into())]));
    }
    for (vname, role) in [("AutoShareServer", "서버"), ("AutoShareWks", "워크스테이션")] {
        let auto = reg_pick(system, "\\lanmanserver\\parameters", vname);
        if auto == "0" {
            rows.push(rf_row(&[("category", "공유 폴더".into()), ("name", format!("관리 공유({})", vname)), ("value", "0 (사용 안 함)".into()), ("status", "정보".into()),
                ("detail", format!("기본 관리 공유(C$, ADMIN$)가 비활성화되어 있습니다. ({})", role)),
                ("key_path", "…\\LanmanServer\\Parameters".into()), ("source", "SYSTEM".into())]));
        }
    }
    let mut seen: HashSet<String> = HashSet::new();
    for r in system {
        if !r.get("key_path").map(|k| k.to_lowercase().ends_with("\\lanmanserver\\shares")).unwrap_or(false) { continue; }
        let name = r.get("value_name").cloned().unwrap_or_default();
        if name.is_empty() || name == "(default)" || seen.contains(&name) { continue; }
        seen.insert(name.clone());
        let vd = r.get("value_data").cloned().unwrap_or_default();
        let path = share_path(&vd);
        let value = if !path.is_empty() { path } else { vd.chars().take(80).collect() };
        rows.push(rf_row(&[("category", "공유 폴더".into()), ("name", name), ("value", value), ("status", "주의".into()),
            ("detail", "사용자 정의 공유 폴더 — 외부 노출/권한 점검 필요".into()),
            ("key_path", r.get("key_path").cloned().unwrap_or_default()), ("source", "SYSTEM".into()),
...
fn mssql_year(ver: &str) -> Option<&'static str> {
    match ver { "10" => Some("2008"), "11" => Some("2012"), "12" => Some("2014"), "13" => Some("2016"),
        "14" => Some("2017"), "15" => Some("2019"), "16" => Some("2022"), "17" => Some("2025"), _ => None }
}
fn sql_instance_label(inst: &str) -> String {
    if inst.is_empty() { return String::new(); }
    let (ver_part, name) = match inst.split_once('.') { Some((v, n)) => (v, n), None => (inst, "") };
    let year = mssql_year(&ver_part.to_uppercase().replace("MSSQL", ""));
    let label = if !name.is_empty() { name } else { inst };
    match year { Some(y) => format!("{} · SQL Server {}", label, y), None => label.to_string() }
}
...
fn rf_sql_auth(software: &[Row]) -> Vec<Row> {
    ...
    for r in software {
        let kp = r.get("key_path").cloned().unwrap_or_default();
        if r.get("value_name").map(|v| v != "LoginMode").unwrap_or(true) || !kp.to_lowercase().contains("microsoft sql server") { continue; }
        let val = r.get("value_data").cloned().unwrap_or_default();
        let inst = kp.split('\\').find(|p| p.to_uppercase().starts_with("MSSQL") && p.contains('.')).unwrap_or("").to_string();
        let label = sql_instance_label(&inst);
        let mixed = val == "2";
        rows.push(rf_row(&[
            ("category", "SQL 인증".into()), ("name", if !label.is_empty() { format!("LoginMode — {}", label) } else { "LoginMode".into() }),
            ("value", match val.as_str() { "1" => "Windows 인증 전용".into(), "2" => "혼합 모드 (SQL+Windows)".into(), _ => format!("알 수 없음({})", val) }),
            ("status", if mixed { "주의".into() } else if val == "1" { "정상".into() } else { "정보".into() }),
            ("detail", if mixed { "혼합 모드 — sa 등 SQL 계정 로그인 사용 가능 (무차별 대입 표적)".into() } else if val == "1" { "Windows 인증만 허용".into() } else { "LoginMode 값 확인 필요".into() }),
            ("key_path", kp), ("source", "SOFTWARE".into())]));
    }
    rows
}

const AUTORUN_SUFFIXES: &[&str] = &["\\currentversion\\run", "\\currentversion\\runonce", "\\policies\\explorer\\run"];

fn rf_autoruns(hives: &[(String, Vec<Row>)]) -> Vec<Row> {
    let mut rows = Vec::new();
    for (fname, reg) in hives {
        let user = hive_user(fname);
        for r in reg {
            let low = r.get("key_path").cloned().unwrap_or_default().to_lowercase();
            if !AUTORUN_SUFFIXES.iter().any(|s| low.ends_with(s)) { continue; }
            let name = r.get("value_name").cloned().unwrap_or_default();
            if name.is_empty() || name == "(default)" { continue; }
            let cmd = r.get("value_data").cloned().unwrap_or_default();
            let kind = if low.ends_with("runonce") { "RunOnce" } else if low.contains("policies") { "Policy Run" } else { "Run" };
            rows.push(rf_row(&[("category", "자동 실행".into()), ("name", name), ("value", cmd.clone()), ("status", "정보".into()), ("detail", kind.into()),
...
fn rf_execution_traces(hives: &[(String, Vec<Row>)]) -> Vec<Row> {
    let mut rows = Vec::new();
    for (fname, reg) in hives {
        let user = hive_user(fname);
        for r in reg {
            let low = r.get("key_path").cloned().unwrap_or_default().to_lowercase();
            let name = r.get("value_name").cloned().unwrap_or_default();
            if name.is_empty() || name == "(default)" || name == "MRUList" || name == "MRUListEx" { continue; }
            let data = r.get("value_data").cloned().unwrap_or_default();
...
fn rf_shimcache(out_dir: &Path) -> Vec<Row> {
    let mut rows = Vec::new();
    let mut seen: HashSet<String> = HashSet::new();
    for (fname, reg) in registry_hives_named(out_dir) {
        if fname.to_uppercase() != "SYSTEM" { continue; }
        for r in reg {
            if r.get("value_name").map(|v| v != "AppCompatCache").unwrap_or(true) { continue; }
            let data = unhex(&r.get("value_data").cloned().unwrap_or_default());
